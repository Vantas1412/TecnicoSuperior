...
const MantenimientoSeccion = () => {
  ...
  // Mapa de empresas por servicio para mostrar la empresa responsable en la lista de tickets.
  ...
  // Cuando cambia la lista de tickets, obtener la empresa asociada a cada servicio
  useEffect(() => {
      ...
      for (const ticket of tickets) {
        ...
        if (idServ && !map[idServ]) {
          try {
            ...
            console.error('Error al obtener empresa para servicio', idServ, err);
          }
        }
      }
  ...
  const handleSubmitTicket = async (e) => {
    ...
    try {
      ...
      if (resultado.success) {
        // Obtener la empresa encargada del servicio solicitado
        try {
          const empresaResult = await empresaService.obtenerEmpresaPorServicio(nuevoTicket.id_servicio);
          if (empresaResult.success) {
            setSuccess(`Su solicitud ha sido aceptada por el servicio técnico de ${empresaResult.data.nombre}.`);
          } else {
            setSuccess('Ticket creado exitosamente. Será atendido por el equipo de mantenimiento.');
          }
        } catch (e) {
          // Si ocurre algún error al obtener la empresa, mostrar mensaje genérico
          console.error('Error al obtener empresa para el servicio:', e);
          setSuccess('Ticket creado exitosamente. Será atendido por el equipo de mantenimiento.');
        }
                    ...
                    {ticket.empleado ? (
                      ...
                      <div className="mt-2 p-2 bg-gray-50 rounded text-xs">
                        {empresasPorServicio[ticket.id_servicio] ? (
                          <>
                            <span className="font-medium">Empresa encargada:</span>{' '}
                            {empresasPorServicio[ticket.id_servicio]}
                          </>
                        ) : (
                          <>
                            <span className="font-medium">Empresa encargada:</span> No asignada
                          </>
                        )}
                      </div>
                    )}
                ...
                <div className="flex justify-between items-center pt-3 border-t border-gray-100">
                  ...
                  {ticket.estado === 'Pendiente' && (
                    <div className="text-xs text-orange-600 font-medium">
                      Esperando atención por la empresa
                    </div>
                  )}
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
        <div className="bg-blue-50 p-4 rounded-lg">
          ...
          <ul className="text-blue-700 space-y-1">
            ...
            <li>3. La empresa se encargará de tu solicitud</li>
